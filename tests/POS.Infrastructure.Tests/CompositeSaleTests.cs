...
public class CompositeSaleTests : IDisposable
{
    ...
    private CreateSaleCommandHandler Handler() =>
        new(_items, _sales, new FakeReceiptNumberGenerator(), _uow,
            new FakeCurrentUser { Id = _userId, Role = "Cashier" },
            _composites, _shifts, _paymentMethods);

    private static CreateSaleCommand Sale(params (Guid id, int qty)[] lines) =>
        new(
